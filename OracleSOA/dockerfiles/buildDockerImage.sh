...
# Description: script to build a Docker image for Oracle SOA suite. The install mode is SOA Suite alone, BPM is not included
#
#
...
if [ $? -eq 0 ]; then
cat << EOF
  Oracle SOA suite install only Docker Image for version: $VERSION is ready to be extended.
...
EOF
  ...
  echo "Oracle SOA Docker Image was NOT successfully created. Check the output and correct any reported problems with the docker build operation."
fi
